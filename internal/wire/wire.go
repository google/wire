...
func Generate(ctx context.Context, wd string, env []string, patterns []string, opts *GenerateOptions) ([]GenerateResult, []error) {
	...
	for i, pkg := range pkgs {
		...
		goSrc := g.frame(isInGoPath(pkg.PkgPath), opts.Tags)
...
// Detect if package is in GOPATH
func isInGoPath(packagePath string) bool {
	goPath := os.Getenv("GOPATH")
	if len(goPath) == 0 {
		return false
	}

	rel, err := filepath.Rel(goPath, packagePath)
	if err != nil {
		return false
	}
...
func (g *gen) frame(isGoPath bool, tags string) []byte {
	if g.buf.Len() == 0 {
		return nil
	}
	var buf bytes.Buffer
	goGenerateCmd := "go run github.com/google/wire/cmd/wire"

	// github.com/google/wire/cmd/wire is not available if project is outside GOPATH
	// so switch back to global executable
	if !isGoPath {
		goGenerateCmd = "wire"
	}
	if len(tags) > 0 {
		// TODO: Check quotes
		goGenerateCmd = fmt.Sprintf("%s gen -tags \"%s\"", goGenerateCmd, tags)
	}
	buf.WriteString("// Code generated by Wire. DO NOT EDIT.\n\n")
	buf.WriteString("//go:generate " + goGenerateCmd + "\n")
